        ...
        try{
            ...
            if (file_exists($filePath)) {
                ...
                if (!in_array($extension, $allowed_types)) {
                    ...
                    if($extension == 'xls'){
                        require_once 'vendor/shuckkin/simplexls/src/SimpleXLS.php';
                        $xlsx = new SimpleXLS($fileContents);
                        return response() -> json(["status"=>"OK","detail"=>"file type .xls ."]);
                    }
                    elseif($extension == 'xlsx'){
                        require_once 'vendor/shuckkin/simplexlsx/src/SimpleXLSX.php';
                        $xlsx = new SimpleXLSX($fileContents);
                        return response() -> json(["status"=>"OK","detail"=>"file type .xlsx ."]);
                    }
                    // print_r($xlsx);
